#!/usr/bin/env python3
"""Train the fecal segmentation model (V2), reusing V1's leakage-safe split.

    python scripts/build_fecal_seg_manifest.py --zenodo <...> --out poultryai/tasks/fecal_seg/manifest.csv
...
Prereqs: pip install -e ".[dev,track]" torchvision. Plain-PyTorch loop (no Lightning).
Loss = weighted cross-entropy + soft Dice. Selects on foreground mIoU; final test report
includes segmentation metrics and a derived image-level disease report (comparable to V1).
"""
...
from poultryai.tasks.fecal_seg import N_SEG_CLASSES
from poultryai.tasks.fecal_seg.data import (make_torch_seg_dataset, read_seg_manifest,
                                            seg_class_pixel_weights)
from poultryai.tasks.fecal_seg.eval import (dominant_class, format_seg_metrics,
...
def evaluate(model, loader, device, smoke=False):
    ...
    for bi, (x, y) in enumerate(loader):
        ...
        for p, t in zip(pred, yt):
            img_pred.append(dominant_class(p))
            img_true.append(dominant_class(t))
        if smoke and bi >= 2:
            break
    m = seg_metrics(conf)
    # derived image-level disease report over the 4 disease classes (drop background rows)
    ip, it = np.array(img_pred), np.array(img_true)
    keep = it > 0
    if keep.any():
        probs = np.zeros((keep.sum(), 4))
        probs[np.arange(keep.sum()), np.clip(ip[keep] - 1, 0, 3)] = 1.0
        m["image_level"] = classification_report(probs, it[keep] - 1)
    return m


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--config", default="configs/fecal_seg.yaml")
    ...
    if t.get("class_weighted_loss", True):
        w = seg_class_pixel_weights(d["manifest"], "train", sample=30 if args.smoke else 300)
        weight = torch.tensor(w, device=device)
        print("pixel class weights:", [round(float(x), 3) for x in w])
    ...
    for epoch in range(epochs):
        ...
        print(f"epoch {epoch+1}/{epochs}  loss={running/max(nb,1):.4f}  "
              f"val_mIoU_fg={sel:.4f}  ({time.time()-t0:.0f}s)")
        if sel > best:
    ...
    if "image_level" in tm:
        print("\n=== TEST (derived image-level disease) ===\n" + format_report(tm["image_level"]))
